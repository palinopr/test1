...
class QualificationState(TypedDict):
    ...
    qualification_status: (
        str  # "initial", "qualifying", "qualified", "not_qualified", "completed"
    )
    ...
    conversation_stage: (
        str  # "greeting", "discovery", "qualification", "presentation", "closing"
    )
...
class CustomerQualificationAgent:
    ...
    def _initialize_llm(self):
        ...
        self.llm = ChatOpenAI(
            model=self.model_name, temperature=0.7, api_key=api_key
        ).bind_tools(self.tools)

        logger.info(
            "LLM initialized", model=self.model_name, tools_count=len(self.tools)
        )
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            {"tools": "tools", "continue": "analyze_response"},
        )
    ...
    def _agent_node(self, state: QualificationState) -> Dict[str, Any]:
        ...
        system_prompt = self._create_system_prompt(
            conversation_stage, contact_info, state
        )
    ...
    def _analyze_response_node(self, state: QualificationState) -> Dict[str, Any]:
        ...
        if any(
            ...
            for indicator in ["employees", "team", "staff", "people"]
        ):
        ...
        if any(
            indicator in response_text
            for indicator in ["revenue", "sales", "income", "profit"]
        ):
        ...
        if any(
            indicator in response_text
            for indicator in self.qualification_criteria["budget_indicators"]
        ):
        ...
        if any(
            indicator in response_text
            for indicator in self.qualification_criteria["timeline_indicators"]
        ):
    ...
    def _create_system_prompt(
        self, stage: str, contact_info: Dict[str, Any], state: QualificationState
    ) -> str:
...
"""
    ...
    async def process_message(
        ...
        try:
            ...
            if not thread_id:
                thread_id = (
                    f"contact_{contact_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
            ...
            return {
                ...
                or "I'm here to help! Could you tell me more about your business?",
    ...
    def get_qualification_summary(self, thread_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "qualification_status": state.values.get(
                    "qualification_status", "initial"
                ),
                "conversation_stage": state.values.get(
                    "conversation_stage", "greeting"
                ),
