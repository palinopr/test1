...
class MetaWebhookHandler:
    ...
    def verify_webhook_signature(self, payload: bytes, signature: str) -> bool:
        ...
        """
        if not self.app_secret:
            logger.warning("Webhook signature validation skipped - no app secret configured")
            return True
        ...
        try:
            ...
            if not is_valid:
                logger.error("Invalid webhook signature", provided=signature[:10] + "...")
    ...
    def extract_lead_data(self, webhook_payload: Dict[str, Any]) -> List[MetaLeadData]:
        ...
        try:
                ...
                for change in changes:
                    if change.get("field") == "leadgen":
                        ...
                        lead_data = MetaLeadData(
                            ...
                            created_time=value.get("created_time", datetime.utcnow().isoformat()),
            ...
            raise HTTPException(status_code=500, detail=f"Lead extraction error: {str(e)}")
    ...
    async def find_or_create_ghl_contact(self, lead_data: MetaLeadData, normalized_fields: Dict[str, str]) -> Optional[str]:
        """
        ...
        try:
            ...
            if email:
                ...
                if search_data.get("success") and search_data.get("count", 0) > 0:
                    ...
                    if contacts:
                        ...
                        logger.info("Found existing contact by email", contact_id=contact_id, email=email)
            ...
            if not contact_id and phone:
                ...
                if search_data.get("success") and search_data.get("count", 0) > 0:
                    ...
                    if contacts:
                        contact_id = contacts[0].get("id")
                        logger.info("Found existing contact by phone", contact_id=contact_id, phone=phone)
            ...
            if contact_id:
                ...
                if update_response.get("success"):
                    logger.info("Updated existing contact with Meta lead data", contact_id=contact_id)
                else:
                    logger.error("Failed to update contact", contact_id=contact_id, error=update_response.get("error"))
            ...
            logger.error("Error finding/creating GHL contact", error=str(e), lead_id=lead_data.id)
            return None
    ...
    async def trigger_qualification_agent(
        ...
        try:
            ...
            if result.get("response"):
                ...
                logger.error("No response from qualification agent", contact_id=contact_id)
                return False
                
        except Exception as e:
            logger.error("Error triggering qualification agent", error=str(e), contact_id=contact_id)
            return False
    
    def _create_initial_message(self, lead_data: MetaLeadData, normalized_fields: Dict[str, str]) -> str:
        """
    ...
    async def process_lead(self, lead_data: MetaLeadData) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("Processing Meta lead", lead_id=lead_data.id, campaign=lead_data.campaign_name)
            
            # Normalize lead fields
            normalized_fields = self.normalize_lead_fields(lead_data.field_data)
            
            if not normalized_fields.get("email") and not normalized_fields.get("phone"):
                return {
                    "success": False,
            ...
            contact_id = await self.find_or_create_ghl_contact(lead_data, normalized_fields)
            ...
            agent_triggered = await self.trigger_qualification_agent(contact_id, lead_data, normalized_fields)
...
async def verify_webhook(request: Request) -> JSONResponse:
    ...
    try:
        ...
        logger.info("Webhook verification request", mode=mode, token_provided=bool(token))
        ...
        if mode == "subscribe" and token == webhook_handler.verify_token:
            ...
            logger.error("Webhook verification failed", expected_token=bool(webhook_handler.verify_token))
...
async def handle_webhook(request: Request, background_tasks: BackgroundTasks) -> JSONResponse:
    ...
    try:
        ...
        if not leads:
            ...
            return JSONResponse(content=WebhookResponse(
                success=True,
        ...
        for lead in leads:
            try:
                ...
                logger.error("Error queuing lead processing", error=str(e), lead_id=lead.id)
        ...
        logger.info("Webhook processing completed", processed_leads=processed_count, errors_count=len(errors))
