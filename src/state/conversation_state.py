...
class ConversationState:
    ...
    def _calculate_qualification_score(self) -> None:
        ...
        if self.business_info.monthly_revenue:
            ...
            if any(
                indicator in revenue_str for indicator in ["100k", "50k", "six figure"]
            ):
                score += 4
        ...
        if self.qualification_data.timeline:
            if (
                "asap" in self.qualification_data.timeline.lower()
                or "urgent" in self.qualification_data.timeline.lower()
            ):
                score += 3
    ...
    def update_metrics(
        self, message_count_delta: int = 0, tools_used: List[str] = None
    ) -> None:
        ...
        self.metrics.engagement_score = min(
            base_engagement + stage_bonus + qualification_bonus, 10.0
        )
...
class ConversationStateManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        self.db_path = db_path or os.getenv(
            "DATABASE_URL", "sqlite:///./conversation_states.db"
        )
    ...
    def _init_database(self) -> None:
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_last_activity ON conversation_states(last_activity)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_qualification_status ON conversation_states(qualification_status)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_customer_email ON conversation_states(customer_email)"
            )
    ...
    def get_state(self, thread_id: str) -> Optional[ConversationState]:
        ...
        try:
            ...
            logger.error(
                "Error loading conversation state", thread_id=thread_id, error=str(e)
            )
            return None
    ...
    def create_state(
        self, thread_id: str, contact_id: str, customer_data: Dict[str, Any]
    ) -> ConversationState:
        ...
        state = ConversationState(thread_id=thread_id, customer_info=customer_info)

        # Save to database
        self.save_state(state)

        logger.info(
            "New conversation state created", thread_id=thread_id, contact_id=contact_id
        )
        return state
    ...
    def _add_to_cache(self, thread_id: str, state: ConversationState) -> None:
        ...
        if len(self.cache) >= self.cache_max_size:
            ...
            oldest_thread = min(
                self.cache.keys(), key=lambda k: self.cache[k].last_activity
            )
    ...
    def cleanup_old_states(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            logger.info(
                "Cleaned up old conversation states",
...
def create_conversation_state(
    thread_id: str, contact_id: str, customer_data: Dict[str, Any]
) -> ConversationState:
    """
